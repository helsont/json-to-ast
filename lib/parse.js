...
function parseObject(input, tokenList, index, settings, parent) {
		...
		switch (state) {
			...
			case objectStates.OPEN_OBJECT: {
				if (token.type === tokenTypes.RIGHT_BRACE) {
					...
					const property = parseProperty(input, tokenList, index, settings, object);
...
function parseProperty(input, tokenList, index, settings, parent) {
	...
	while (index < tokenList.length) {
		...
		switch (state) {
			...
			case propertyStates.COLON: {
				const value = parseValue(input, tokenList, index, settings, property);
...
function parseArray(input, tokenList, index, settings, parent) {
		...
		switch (state) {
			...
			case arrayStates.OPEN_ARRAY: {
				if (token.type === tokenTypes.RIGHT_BRACKET) {
					...
					const value = parseValue(input, tokenList, index, settings, array);
					index = value.index;
...
function parseLiteral(input, tokenList, index, settings, parent) {
...
function parseValue(input, tokenList, index, settings, parent) {
...
export default (input, settings) => {
	...
	const value = parseValue(input, tokenList, 0, settings, null);
